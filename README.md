...
> 🏢 **会社としての全体像は [`CLAUDE.md`](CLAUDE.md) と [`business/`](business/README.md) を参照してください。**
> 私たちは「ママを不安から解放し、子育ては喜びだと思い出してもらう」ことを使命に、
> AIと共同で会社を経営しています。最初のプロダクトは赤ちゃん睡眠アプリ（仮称「すやすや」）。
> 本システムはその**グロース（集客）の武器＝コンテンツ自動生成エンジン**として位置づけられます。
...
### ステップ3.5（任意）: APIキー無しで動作確認（おすすめ）

セットアップが正しくできたか、APIキーを入れる前に確認したい場合は
**オフラインデモ** を実行できます。サンプルテーマ「乳児の便秘」で、記事・スライド・
監修Excelまで一式が `output/` に生成されます（Web通信・課金なし）。

```bash
python main.py --demo
```

問題なく `✅ 全工程完了` と表示されれば、ライブラリのインストールは成功しています。
...
```bash
python main.py --demo                   # APIキー不要のオフラインデモ（動作確認用）
